...
/*　　　ＰＧＭ名　　　　：計画終了対象商品日次更新通知
/*　　　ＰＧＭID　　　　：hnadpl450.sql
/*　　　機能　　　　　　：HNT_更新通知のデータ区分「04」レコードを更新する
/*　　　　　　　　　　　：
...
---------------------------------------------
    ...
    dbms_output.put_line('**** HNADPL450 START : ' || w_time_start);
    :RtnCd := 0;
...
---------------------------------------------
    ...
    dbms_output.put_line('**** HNADPL450  END  : ' || w_time_end);
...
EXCEPTION
    ...
    -- ErrMsg SET
        :ErrMsg := substr(sqlerrm,1,100);
    -- ログ出力
        dbms_output.put_line(to_char(sysdate,'YYYY-MM-DD HH24:MI:SS') || ' HNADPL450 ABNORMAL ENDED' || ')');
        dbms_output.put_line('**** ORACLE ERROR MESSAGE  :' || :ErrMsg);
        dbms_output.put_line('**** HNADPL450 RETURN CODE :' || :RtnCd);
