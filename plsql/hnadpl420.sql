/********************************************************************************/
/*　　　システム名　　　：ＤＤＣ返品コスト削減
/*　　　ＰＧＭ名　　　　：在庫切れ速報更新通知２２時締め
/*　　　ＰＧＭID　　　　：hnadpl420.sql
/*　　　機能　　　　　　：HNT_更新通知のデータ区分「06」レコードを更新する
/*　　　　　　　　　　　：
...
---------------------------------------------
    ...
    dbms_output.put_line('**** HNADPL420 START : ' || w_time_start);
    :RtnCd := 0;
...
---------------------------------------------
    ...
    dbms_output.put_line('**** HNADPL420  END  : ' || w_time_end);
...
EXCEPTION
    ...
    -- ErrMsg SET
        :ErrMsg := substr(sqlerrm,1,100);
    -- ログ出力
        dbms_output.put_line(to_char(sysdate,'YYYY-MM-DD HH24:MI:SS') || ' HNADPL420 ABNORMAL ENDED' || ')');
        dbms_output.put_line('**** ORACLE ERROR MESSAGE  :' || :ErrMsg);
        dbms_output.put_line('**** HNADPL420 RETURN CODE :' || :RtnCd);
