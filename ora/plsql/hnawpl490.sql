...
/*　　　ＰＧＭ名　　　　：計画終了対象商品週次更新通知
/*　　　ＰＧＭID　　　　：hnawpl490.sql
/*　　　機能　　　　　　：HNT_更新通知のデータ区分「05」レコードを更新する
/*　　　　　　　　　　　：
...
---------------------------------------------
    ...
    dbms_output.put_line('**** HNAWPL490 START : ' || w_time_start);
    :RtnCd := 0;
...
---------------------------------------------
    ...
    dbms_output.put_line('**** HNAWPL490  END  : ' || w_time_end);
...
EXCEPTION
    ...
    -- ErrMsg SET
        :ErrMsg := substr(sqlerrm,1,100);
    -- ログ出力
        dbms_output.put_line(to_char(sysdate,'YYYY-MM-DD HH24:MI:SS') || ' HNAWPL490 ABNORMAL ENDED' || ')');
        dbms_output.put_line('**** ORACLE ERROR MESSAGE  :' || :ErrMsg);
        dbms_output.put_line('**** HNAWPL490 RETURN CODE :' || :RtnCd);
