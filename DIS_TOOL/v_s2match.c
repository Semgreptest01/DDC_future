...
/*      バイトとする。                                                      */
/*  使用法      ：  v_s2match [+stderr] pk=$$ master < tran > output      */
/*  ステータス  ：  0 : 正常終了                                            */
/*                  1 : 警告終了（出力失敗データ有り）                      */
/*                  2 : アーギュメント不正                                  */
/*                  3 : ファイルオープンエラー                              */
...
/*  002 2002/07/03  木下    エラー出力をマッチした時も行っていた為、        */
/*                          トランオンリー時のみエラー出力するようにした    */
/*　004 2002/11/21　川村 　 最大レコードサイズ変更(1024Byte=>3072byte)      */
/*                          １レコード最大フィールド数を４００に変更        */
/*　005 2002/11/28　川村 　 マッチングキーソート順指定対応。                */
/*                          マッチングキー比較文字数指定対応。              */
...
char mst_buf[MAX_FSNUM][MAX_RECLEN];    /* マスターフィールドＴＢＬ     */
char trn_buf[MAX_FSNUM][MAX_RECLEN];    /* トランフィールドＴＢＬ       */
...
/*          printf("key_tbl[1]=%d\n", key_tbl[0]);            ** DEBUG */
            ...
            if (key_data[loop_cnt].cutnum == 0) {
                ...
                /* 比較文字数が指定されていれば、該当文字数分コピー         */
                strncat(cat_char, mst_buf[key_data[loop_cnt].order - key_tbl[0]],
                        key_data[loop_cnt].cutnum);
            }
/*          printf("mst_char=%s\n",cat_char);                 ** DEBUG */
        }
...
/*  printf("w_pk_cnt=%d\n", w_pk_cnt);                         ** DEBUG */
    ...
    /*  トランオンリー                                                      */
        ...
        /*  出力バッファに格納                                              */
        /*////////////////////////////////////////////////////////////////// */
        strcpy(output_buf, trn_all_rec);

#ifdef  NEW_TOKEN
        output_len = strlen(output_buf);
        if (output_buf[output_len - 1] != '\n') {
            output_buf[output_len++] = '\n';
        }
        fwrite(output_buf, output_len, 1, stderr);
#endif
    }
...
void usage()
{
    puts("使用方法: v_s2match [+stderr] pk=$$ masterfile < tranfile > outputfile");
    exit(ARG_ERR);
}
