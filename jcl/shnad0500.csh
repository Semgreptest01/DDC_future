...
# 修正履歴      : 
# NO  修正日付     修正者      修正内容
#  1  2025/12/12   R.Someya    共通ソースパスを変更
#------------------------------------------------------------------------#
# 初期処理部
#------------------------------------------------------------------------#
...
# ロードファイル存在チェック処理
#------------------------------------------------------------------------#
set DAT_FILE = "$WKDIR/hnt224p_001.dat"
...
#------------------------------------------------------------------------#
# DROP PRIMARY KEY HNT_オンライン宛先ベンダー
#------------------------------------------------------------------------#
set DDL_FILE    = $SQLDIR/hnt012_pk_drp_con_${ORAUID}.sql
...
#------------------------------------------------------------------------#
# TRUNCATE HNT_オンライン宛先ベンダー
#------------------------------------------------------------------------#
set DDL_FILE    = $SQLDIR/hnt012_trc_tbl_${ORAUID}.sql
set SQL_NAME    = sqlplus
set SQL_PARM    = (-s $ORAUID/$ORAPWD @$DDL_FILE)
set CHECKFILE   = $DDL_FILE
source $SUBSRCDIR/filechk.src
...
#------------------------------------------------------------------------#
# SQL*Loader HNT_オンライン宛先ベンダー
#------------------------------------------------------------------------#
set CTL_FILE = "$CTLDIR/hnt012.ctl"
...
#------------------------------------------------------------------------#
# ADD PRIMARY KEY HNT_オンライン宛先ベンダー
#------------------------------------------------------------------------#
set DDL_FILE    = $SQLDIR/hnt012_pk_add_con_${ORAUID}.sql
set SQL_NAME    = sqlplus
set SQL_PARM    = (-s $ORAUID/$ORAPWD @$DDL_FILE)
set CHECKFILE   = $DDL_FILE
source $SUBSRCDIR/filechk.src
...
#------------------------------------------------------------------------#
# ANALYZE HNT_オンライン宛先ベンダー
#------------------------------------------------------------------------#
set DDL_FILE    = $SQLDIR/hnt012_alz_tbl_${ORAUID}.sql
set SQL_NAME    = sqlplus
set SQL_PARM    = (-s $ORAUID/$ORAPWD @$DDL_FILE)
set CHECKFILE   = $DDL_FILE
source $SUBSRCDIR/filechk.src
