...
# 修正履歴      :
# NO  修正日付     修正者      修正内容
#  1  2025/12/17   K.Takashima 共通ソースパスを変更
#------------------------------------------------------------------------#
# 初期処理部
#------------------------------------------------------------------------#
...
#------------------------------------------------------------------------#
# TRUNCATE HNT_計画終了対象商品＿週次帳票
#------------------------------------------------------------------------#
set DDL_FILE    = $SQLDIR/hnt037_trc_tbl_${ORAUID}.sql
...
#------------------------------------------------------------------------#
#   PLSQL ( hnawpl540.sql ) 計画終了対象商品週次帳票作成
#------------------------------------------------------------------------#
set JN_SQL = $PLSQLDIR/hnawpl540.sql
set SQL_NAME = sqlplus
...
#------------------------------------------------------------------------#
# ANALYZE HNT_計画終了対象商品＿週次帳票
#------------------------------------------------------------------------#
set DDL_FILE    = $SQLDIR/hnt037_alz_tbl_${ORAUID}.sql
set SQL_NAME    = sqlplus
set SQL_PARM    = (-s $ORAUID/$ORAPWD @$DDL_FILE)
set CHECKFILE   = $DDL_FILE
source $SUBSRCDIR/filechk.src
