...
#                                   ベンダー別計画終了対象商品受信処理に変更
#  3  2025/12/12   R.Someya       共通ソースパスを変更
#------------------------------------------------------------------------#
# 初期処理部
#------------------------------------------------------------------------#
...
# ロードファイル存在チェック処理
#------------------------------------------------------------------------#
set DAT_FILE = "$WKDIR/hnt837p_001.dat"
...
#------------------------------------------------------------------------#
# DROP PRIMARY KEY HNT_ベンダー別計画終了対象商品
#------------------------------------------------------------------------#
set DDL_FILE    = $SQLDIR/hnt040_pk_drp_con_${ORAUID}.sql
...
#------------------------------------------------------------------------#
# TRUNCATE HNT_ベンダー別計画終了対象商品
#------------------------------------------------------------------------#
set DDL_FILE    = $SQLDIR/hnt040_trc_tbl_${ORAUID}.sql
set SQL_NAME    = sqlplus
set SQL_PARM    = (-s $ORAUID/$ORAPWD @$DDL_FILE)
set CHECKFILE   = $DDL_FILE
source $SUBSRCDIR/filechk.src
...
#------------------------------------------------------------------------#
# SQL*Loader HNT_ベンダー別計画終了対象商品
#------------------------------------------------------------------------#
set CTL_FILE = "$CTLDIR/hnt040.ctl"
...
#------------------------------------------------------------------------#
# ADD PRIMARY KEY HNT_ベンダー別計画終了対象商品
#------------------------------------------------------------------------#
set DDL_FILE    = $SQLDIR/hnt040_pk_add_con_${ORAUID}.sql
set SQL_NAME    = sqlplus
set SQL_PARM    = (-s $ORAUID/$ORAPWD @$DDL_FILE)
set CHECKFILE   = $DDL_FILE
source $SUBSRCDIR/filechk.src
...
#------------------------------------------------------------------------#
# ANALYZE HNT_ベンダー別計画終了対象商品
#------------------------------------------------------------------------#
set DDL_FILE    = $SQLDIR/hnt040_alz_tbl_${ORAUID}.sql
set SQL_NAME    = sqlplus
set SQL_PARM    = (-s $ORAUID/$ORAPWD @$DDL_FILE)
set CHECKFILE   = $DDL_FILE
source $SUBSRCDIR/filechk.src
