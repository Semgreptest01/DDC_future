...
#                                   入出残セットマスタ受信処理に変更
#  3  2025/12/16   K.Takashima    共通ソースパスを変更
#------------------------------------------------------------------------#
# 初期処理部
#------------------------------------------------------------------------#
...
# ロードファイル存在チェック処理
#------------------------------------------------------------------------#
set DAT_FILE = "$WKDIR/hnt835p_001.dat"
...
#------------------------------------------------------------------------#
# DROP PRIMARY KEY HNT_入出残セットマスタ
#------------------------------------------------------------------------#
set DDL_FILE    = $SQLDIR/hnt039_pk_drp_con_${ORAUID}.sql
...
#------------------------------------------------------------------------#
# TRUNCATE HNT_入出残セットマスタ
#------------------------------------------------------------------------#
set DDL_FILE    = $SQLDIR/hnt039_trc_tbl_${ORAUID}.sql
set SQL_NAME    = sqlplus
set SQL_PARM    = (-s $ORAUID/$ORAPWD @$DDL_FILE)
set CHECKFILE   = $DDL_FILE
source $SUBSRCDIR/filechk.src
...
#------------------------------------------------------------------------#
# SQL*Loader HNT_入出残セットマスタ
#------------------------------------------------------------------------#
set CTL_FILE = "$CTLDIR/hnt039.ctl"
...
#------------------------------------------------------------------------#
# ADD PRIMARY KEY HNT_計画終了対象商品
#------------------------------------------------------------------------#
set DDL_FILE    = $SQLDIR/hnt039_pk_add_con_${ORAUID}.sql
set SQL_NAME    = sqlplus
set SQL_PARM    = (-s $ORAUID/$ORAPWD @$DDL_FILE)
set CHECKFILE   = $DDL_FILE
source $SUBSRCDIR/filechk.src
...
#------------------------------------------------------------------------#
# ANALYZE HNT_入出残セットマスタ
#------------------------------------------------------------------------#
set DDL_FILE    = $SQLDIR/hnt039_alz_tbl_${ORAUID}.sql
set SQL_NAME    = sqlplus
set SQL_PARM    = (-s $ORAUID/$ORAPWD @$DDL_FILE)
set CHECKFILE   = $DDL_FILE
source $SUBSRCDIR/filechk.src
