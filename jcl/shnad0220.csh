...
# 修正履歴      : 
# NO  修正日付     修正者      修正内容
# 01  2014/02/14   M.Nakazawa  受信ファイル圧縮対応
# 02  2014/02/15   S.Furuya    受信ファイル圧縮対応
# 03  2025/11/17   R.SOMEYA    LMDC移行対応：ftpass=>IFP
# 04  2025/12/12   R.Someya    共通ソースパスを変更
#------------------------------------------------------------------------#
# 初期処理部
#------------------------------------------------------------------------#
...
#   ファイル削除処理
#------------------------------------------------------------------------#
rm -f $WKDIR/hnt227p_001.dat
rm -f $WKDIR/hnt227p_001.zip
rm -f $WKDIR/WS_HS_VDRBT_SHN.DAT
#
#------------------------------------------------------------------------#
#   ファイル受信
#------------------------------------------------------------------------#
#20251117 LMDC移行
#set F_FILE="$FTPASSTYPE.MST227P.001"
##set D_FILE="$WKDIR/hnt227p_001.dat"
#set D_FILE="$WKDIR/hnt227p_001.zip"
#set N_NO="1"
##
...
set F_FILE    =  ${IFTYPE}HN-AD00-MST227P001     # 論理ファイル名
set D_FILE    =  $WKDIR/hnt227p_001.zip          # 取出し後ファイル名
#
...
#   ファイル権限変更
#------------------------------------------------------------------------#
chmod 777 $WKDIR/hnt227p_001.zip
#
#------------------------------------------------------------------------#
#   ファイル解凍
#------------------------------------------------------------------------#
unzip $WKDIR/hnt227p_001.zip  -d  $WKDIR/
if ( $status != $NORMAL )  then
    echo "$SHELLNAME:t : common.src = end ERR" | tee -a $LOG_FILE
    exit $ABEND
endif
...
#   ファイル権限変更
#------------------------------------------------------------------------#
chmod 777 $WKDIR/WS_HS_VDRBT_SHN.DAT
#
#------------------------------------------------------------------------#
#   ファイル名変更
#------------------------------------------------------------------------#
mv $WKDIR/WS_HS_VDRBT_SHN.DAT $WKDIR/hnt227p_001.dat  >> $LOG_FILE
if ( $status != $NORMAL )  then
    echo "$SHELLNAME:t : common.src = end ERR" | tee -a $LOG_FILE
    exit $ABEND
endif
...
#   ファイル権限変更
#------------------------------------------------------------------------#
chmod 666 $WKDIR/hnt227p_001.dat
#
#------------------------------------------------------------------------#
#   DATE出力
#------------------------------------------------------------------------#
